...
#[test]
fn multi_last_cor1_sepa_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);

    let err = handler
        .new_job("MultiLastCOR1SEPA")
        .expect_err("COR1 bulk direct debit is outside v1 public registry");
    assert_eq!(
        err.message(),
        "unsupported or out-of-scope job: MultiLastCOR1SEPA"
    );
}
...
#[test]
fn last_cor1_sepa_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);

    let err = handler
        .new_job("LastCOR1SEPA")
        .expect_err("COR1 direct debit is outside v1 public registry");
    assert_eq!(
        err.message(),
