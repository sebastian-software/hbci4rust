...
#[test]
fn last_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);

    let err = handler
        .new_job("Last")
        .expect_err("classic direct debit is outside v1 public registry");
    assert_eq!(err.message(), "unsupported or out-of-scope job: Last");
}

#[test]
fn storno_last_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);

    let err = handler
        .new_job("StornoLast")
        .expect_err("classic direct debit objection is outside v1 public registry");
    assert_eq!(err.message(), "unsupported or out-of-scope job: StornoLast");
}
