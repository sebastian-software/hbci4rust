...
fn visual_tan_pintan_data(id: &str, name: &str) -> PinTanPassportData {
    PinTanPassportData {
        tan_method: Some("921".to_owned()),
        bpd_parameters: BTreeMap::from([
            (
            ...
            (
                "Params.TAN2StepPar5.ParTAN2Step.id".to_owned(),
                id.to_owned(),
            ),
        ]),
        ..signed_pintan_data()
...
fn ascii_image_payload() -> String {
    format!("\0\timage/png\0\0{}", "A".repeat(101))
}
...
fn kontoauszug_pdf_exposes_original_near_constraints() {
    ...
    let handler = HbciHandler::new("300", passport);
    let mut job = handler
        .new_job("KontoauszugPdf")
        .expect("job is in registry");

    assert_eq!(job.constraints().len(), 10);
    assert_eq!(
        job.constraint("my.iban")
            .expect("iban constraint")
            .destination_name,
        "KontoauszugPdf2.My.iban"
    );
    assert_eq!(
        job.constraint("my.bic")
            .expect("bic constraint")
            .destination_name,
        "KontoauszugPdf2.My.bic"
    );
    assert_eq!(
        job.constraint("my.country")
            .expect("country constraint")
    ...
    assert_eq!(
        job.constraint("offset")
            .expect("offset constraint")
            .destination_name,
        "KontoauszugPdf2.offset"
    );
    assert_eq!(
        job.constraint("idx")
            .expect("index constraint")
            .default_value
            .as_deref(),
        Some("")
    );
    ...
    assert_eq!(job.lowlevel_param("KontoauszugPdf2.idx"), Some("3"));
    assert_eq!(job.lowlevel_param("KontoauszugPdf2.year"), Some("2024"));
}
...
#[test]
fn dauer_list_exposes_original_near_v5_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let mut job = handler.new_job("DauerList").expect("job is in registry");
    ...
    assert_eq!(
        job.constraint("my.blz")
            .expect("bank code constraint")
            .destination_name,
        "DauerList5.KTV.KIK.blz"
    );
    assert_eq!(
        job.constraint("my.number")
            .expect("account number constraint")
            .destination_name,
        "DauerList5.KTV.number"
    );
    assert_eq!(
        job.constraint("my.subnumber")
            .expect("subnumber constraint")
    ...
    assert_eq!(
        job.lowlevel_param("DauerList5.KTV.number"),
        Some("1234567890")
    );
    assert_eq!(job.lowlevel_param("DauerList5.maxentries"), Some("10"));
}
...
#[test]
fn dauer_edit_exposes_original_near_v5_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let mut job = handler.new_job("DauerEdit").expect("job is in registry");
    ...
    assert_eq!(
        job.constraint("date")
            .expect("scheduled change date constraint")
            .default_value
            .as_deref(),
        Some("")
    );
    assert_eq!(
        job.constraint("lastdate")
            .expect("lastdate constraint")
            .destination_name,
        "DauerEdit5.DauerDetails.lastdate"
    );
    assert_eq!(
        job.constraint("key")
            .expect("transaction key constraint")
    ...
    assert_eq!(
        job.constraint("usage_14")
            .expect("usage_14 constraint")
            .destination_name,
        "DauerEdit5.usage.usage_14"
    );

    job.try_set_param("orderid", "ORDEROLD")
        .expect("orderid is accepted");
    assert_eq!(job.lowlevel_param("DauerEdit5.orderid"), Some("ORDEROLD"));
    job.try_set_param_date("date", "2026-02-01")
        .expect("scheduled change date is accepted");
    assert_eq!(job.lowlevel_param("DauerEdit5.date"), Some("2026-02-01"));
}
...
fn handler_accepts_explicit_dauer_edit_without_snapshot_like_original() {
    ...
    job.try_set_param("usage", "Explicit usage")
...
#[test]
fn multi_last_exposes_original_near_v6_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let mut job = handler.new_job("MultiLast").expect("job is in registry");
    ...
    assert_eq!(
        job.lowlevel_param("SammelLast6.data"),
...
#[test]
fn term_multi_ueb_sepa_exposes_original_near_v1_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let job = handler
        .new_job("TermMultiUebSEPA")
        .expect("job is in registry");

    assert_eq!(job.constraints().len(), 25);
    assert_eq!(
        job.constraint("src.iban")
            .expect("source iban constraint")
            .destination_name,
        "TermSammelUebSEPA1.My.iban"
    );
    assert_eq!(
        job.constraint("_sepadescriptor")
            .expect("sepa descriptor")
    ...
    assert_eq!(
        job.constraint("Total.value")
            .expect("total value constraint")
            .destination_name,
        "TermSammelUebSEPA1.Total.value"
    );
    assert_eq!(
        job.constraint("date")
            .expect("execution date constraint")
            .destination_name,
...
#[test]
fn multi_last_sepa_exposes_original_near_v1_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let job = handler
        .new_job("MultiLastSEPA")
        .expect("job is in registry");
...
#[test]
fn multi_last_b2b_sepa_exposes_original_near_v1_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let job = handler
        .new_job("MultiLastB2BSEPA")
        .expect("job is in registry");
    ...
    assert_eq!(
        job.constraint("Total.value")
            .expect("total value constraint")
            .destination_name,
        "SammelLastB2BSEPA1.Total.value"
    );
    assert_eq!(
        job.constraint("Total.curr")
            .expect("total currency constraint")
            .destination_name,
        "SammelLastB2BSEPA1.Total.curr"
    );
}

#[test]
fn last_exposes_original_near_v5_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let mut job = handler.new_job("Last").expect("job is in registry");
    ...
    assert_eq!(
        job.constraint("type")
            .expect("direct debit type constraint")
            .destination_name,
        "Last5.key"
    );
    assert_eq!(
    ...
    assert_eq!(
        job.constraint("usage_14")
            .expect("usage_14 constraint")
            .destination_name,
        "Last5.usage.usage_14"
    );
    assert!(job.constraint("key").is_none());
    assert!(job.constraint("addkey").is_none());
    assert!(job.constraint("date").is_none());
    assert!(job.constraint("id").is_none());

    job.try_set_param("type", "06")
        .expect("direct debit type is accepted");
    job.try_set_param("usage_2", "Second usage")
        .expect("second usage line is accepted");
    assert_eq!(job.lowlevel_param("Last5.key"), Some("06"));
    assert_eq!(
        job.lowlevel_param("Last5.usage.usage_2"),
        Some("Second usage")
    );
}
...
fn last_sepa_exposes_original_near_v1_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let job = handler.new_job("LastSEPA").expect("job is in registry");
    ...
    assert_eq!(
        job.constraint("sequencetype")
            .expect("sequence type dummy constraint")
            .default_value
            .as_deref(),
        Some("FRST")
    );
    assert_eq!(
        job.constraint("targetdate")
            .expect("target date dummy constraint")
            .default_value
            .as_deref(),
        Some("1999-01-01")
    );
...
#[test]
fn last_cor1_sepa_exposes_original_near_v1_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let job = handler.new_job("LastCOR1SEPA").expect("job is in registry");
...
fn inst_ueb_sepa_exposes_original_near_v1_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let job = handler.new_job("InstUebSEPA").expect("job is in registry");
...
#[test]
fn umb_sepa_exposes_original_near_v1_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let job = handler.new_job("UmbSEPA").expect("job is in registry");

    assert_eq!(job.constraints().len(), 21);
    assert_eq!(
        job.constraint("src.iban")
            .expect("source iban constraint")
            .destination_name,
...
#[test]
fn term_ueb_sepa_exposes_original_near_v1_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let job = handler.new_job("TermUebSEPA").expect("job is in registry");
...
#[test]
fn term_ueb_sepa_list_exposes_original_near_v1_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let job = handler
        .new_job("TermUebSEPAList")
        .expect("job is in registry");

    assert_eq!(job.constraints().len(), 12);
    assert_eq!(
        job.constraint("my.iban")
            .expect("my iban constraint")
            .destination_name,
        "TermUebSEPAList1.My.iban"
    );
    assert_eq!(
        job.constraint("src.iban")
            .expect("source iban alias")
    ...
    assert_eq!(
        job.constraint("startdate")
            .expect("startdate constraint")
            .destination_name,
        "TermUebSEPAList1.startdate"
    );
    assert_eq!(
        job.constraint("enddate")
            .expect("enddate constraint")
            .destination_name,
        "TermUebSEPAList1.enddate"
    );
    assert_eq!(
        job.constraint("maxentries")
            .expect("maxentries constraint")
            .destination_name,
        "TermUebSEPAList1.maxentries"
    );
}

#[test]
fn term_ueb_exposes_original_near_v4_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let mut job = handler.new_job("TermUeb").expect("job is in registry");
    ...
    assert_eq!(
        job.constraint("usage")
            .expect("usage constraint")
            .destination_name,
        "TermUeb4.usage.usage"
    );
    assert_eq!(
        job.constraint("usage_14")
            .expect("usage_14 constraint")
            .destination_name,
        "TermUeb4.usage.usage_14"
    );

    job.try_set_param("usage_2", "Second usage")
        .expect("second usage line is accepted");
    assert_eq!(
        job.lowlevel_param("TermUeb4.usage.usage_2"),
        Some("Second usage")
    );
}
...
#[test]
fn term_ueb_edit_exposes_original_near_v4_constraints() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let mut job = handler.new_job("TermUebEdit").expect("job is in registry");

    assert_eq!(job.constraints().len(), 29);
    assert_eq!(
        job.constraint("src.number")
            .expect("source account number constraint")
            .destination_name,
        "TermUebEdit4.My.number"
    );
    assert_eq!(
        job.constraint("src.country")
            .expect("source country constraint")
    ...
    assert_eq!(
        job.constraint("usage")
            .expect("usage constraint")
            .destination_name,
        "TermUebEdit4.usage.usage"
    );
    assert_eq!(
        job.constraint("usage_14")
            .expect("usage_14 constraint")
            .destination_name,
        "TermUebEdit4.usage.usage_14"
    );

    job.try_set_param("orderid", "ORDEROLD")
        .expect("order id is accepted");
    assert_eq!(job.lowlevel_param("TermUebEdit4.id"), Some("ORDEROLD"));
    job.try_set_param("usage_2", "Second edited usage")
        .expect("second usage line is accepted");
    assert_eq!(
        job.lowlevel_param("TermUebEdit4.usage.usage_2"),
        Some("Second edited usage")
    );
}
...
#[tokio::test]
async fn handler_renders_and_collects_dauer_list_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
        "HIRMG:2:2+0010::OK",
        "HIDAB:3:5+1234567890::280:10020030+99887766::280:20030040+Receiver Name+Receiver Name 2+12,30:EUR+51+000+Usage one:Usage two+20250701+ORDER123+20250101:M:2:15:20261231+J:20250201:20250301:1:5,00:EUR+J+N+J",
    ]))]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler.new_job("DauerList").expect("job is in registry");
    job.try_set_param("my.number", "1234567890")
    ...
    job.try_set_param("my.blz", "10020030")
        .expect("bank code is accepted");
    job.try_set_param("orderid", "ORDER123")
        .expect("order id is accepted");
    job.try_set_param_int("maxentries", 10)
        .expect("max entries is accepted");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "DauerList");
    assert!(status.job_results[0].success);
    assert_eq!(
        status.job_results[0]
            .result_data
            .get("content.BTG.value")
            .map(String::as_str),
        Some("12.30")
    );
    assert_eq!(
    ...
    assert_eq!(
        entry.usage,
        ["Usage one".to_owned(), "Usage two".to_owned()]
    );
    assert_eq!(entry.nextdate.as_deref(), Some("2025-07-01"));
    assert_eq!(entry.orderid.as_deref(), Some("ORDER123"));
    assert_eq!(entry.firstdate.as_deref(), Some("2025-01-01"));
    assert_eq!(entry.timeunit.as_deref(), Some("M"));
    assert_eq!(entry.turnus, Some(2));
    assert_eq!(entry.execday, Some(15));
    ...
    assert_eq!(aussetzung.startdate.as_deref(), Some("2025-02-01"));
    assert_eq!(aussetzung.enddate.as_deref(), Some("2025-03-01"));
    assert_eq!(aussetzung.number.as_deref(), Some("1"));
    assert_eq!(
        aussetzung
            .newvalue
            .as_ref()
    ...
    assert_eq!(snapshot.get("BTG.value").map(String::as_str), Some("12.30"));
    assert_eq!(
    ...
    assert_signed_custom_msg_request(&body, "0", "1", 5);
    assert!(
        body.contains("HKDAB:3:5+1234567890::280:10020030+ORDER123+10'"),
        "{body}"
    );
}
...
#[tokio::test]
async fn handler_renders_and_collects_dauer_edit_like_original() {
    let mut data = signed_pintan_data();
    data.persistent_data.insert(
        "dauer_ORDEROLD".to_owned(),
        BTreeMap::from([
            ...
            ("orderid".to_owned(), "ORDEROLD".to_owned()),
    ...
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
        "HIRMG:2:2+0010::OK",
        "HIDAN:3:5+ORDERNEW+ORDEROLD",
    ]))]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler.new_job("DauerEdit").expect("job is in registry");
    job.try_set_param("orderid", "ORDEROLD")
        .expect("orderid is accepted");
    job.try_set_param("usage", "Edited dauer usage one")
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "DauerEdit");
    assert!(status.job_results[0].success);
    assert_eq!(
        status.job_results[0]
            .result_data
    ...
    assert_eq!(result.order_id.as_deref(), Some("ORDERNEW"));
    assert_eq!(result.order_id_old.as_deref(), Some("ORDEROLD"));
    ...
    assert_eq!(
        ...
        Some("Edited dauer usage one")
    );
    assert_eq!(
        snapshot.get("usage.usage_2").map(String::as_str),
        Some("Dauer usage two")
    );
    assert_eq!(snapshot.get("date").map(String::as_str), Some("2026-02-01"));
    assert_eq!(
        snapshot.get("DauerDetails.execday").map(String::as_str),
        Some("15")
    );
    assert_eq!(snapshot.get("key").map(String::as_str), Some("52"));
    assert!(!snapshot.contains_key("orderid"));
    assert!(!snapshot.keys().any(|key| key.ends_with(".orderid")));
    assert!(!snapshot.keys().any(|key| key.starts_with("Aussetzung.")));
    assert!(
        handler
            .passport()
            .get_persistent_data("dauer_ORDEROLD")
            .is_some()
    );
...
#[tokio::test]
async fn handler_renders_multi_last_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_ok_response())]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler.new_job("MultiLast").expect("job is in registry");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "MultiLast");
    assert!(status.job_results[0].success);
    assert!(status.job_results[0].result.is_none());
    assert!(
        !status.job_results[0]
    ...
    assert!(
        !handler
            ...
            .persistent_data()
            .keys()
            .any(|key| key.starts_with("multilast_"))
    );
    ...
    assert_signed_custom_msg_request(&body, "0", "1", 5);
    assert!(
        body.contains("HKSLA:3:6+1234567890::280:10020030+@16@DTAUS+WITH:COLON'"),
        "{body}"
    );
}
...
#[tokio::test]
async fn handler_renders_and_collects_term_multi_ueb_sepa_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
        "HIRMG:2:2+0010::OK",
        "HICME:3:1+ORDERTERMMULTI",
    ]))]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler
        .new_job("TermMultiUebSEPA")
    ...
    job.try_set_indexed_param("btg.curr", 0, "EUR")
        .expect("first currency is accepted");
    job.try_set_indexed_param("usage", 0, "Term usage one")
        .expect("first usage is accepted");
    job.try_set_indexed_param("endtoendid", 0, "E2E-TERM-1")
        .expect("first end to end id is accepted");
    job.try_set_indexed_param("dst.name", 1, "Receiver Two")
        .expect("second destination name is accepted");
    job.try_set_indexed_param("dst.iban", 1, "DE77123456780000000000")
    ...
    job.try_set_indexed_param("btg.curr", 1, "EUR")
        .expect("second currency is accepted");
    job.try_set_indexed_param("usage", 1, "Term usage two")
        .expect("second usage is accepted");
    job.try_set_indexed_param("endtoendid", 1, "E2E-TERM-2")
        .expect("second end to end id is accepted");
    job.try_set_param("sepaid", "SEPA-TERM-MULTI")
        .expect("sepa id is accepted");
    job.try_set_param("pmtinfid", "PMT-TERM-MULTI")
        .expect("payment info id is accepted");
    job.try_set_param_date("date", "2026-04-20")
        .expect("date is accepted");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "TermMultiUebSEPA");
    assert!(status.job_results[0].success);
    assert_eq!(
        status.job_results[0]
            .result_data
    ...
    let Some(HbciJobResultData::TermUeb(result)) = status.job_results[0].result.as_ref() else {
        panic!("expected TermUeb result data");
    };
    assert_eq!(result.order_id.as_deref(), Some("ORDERTERMMULTI"));

    let snapshot = handler
        .passport()
        .get_persistent_data("termueb_ORDERTERMMULTI")
        .expect("term multi ueb sepa persistent data");
    assert_eq!(
        snapshot.get("My.iban").map(String::as_str),
        Some("DE02123456780000000000")
    );
    ...
    let generated_pain = snapshot
        ...
        .expect("generated pain is persisted");
    assert!(generated_pain.starts_with("B<?xml"), "{generated_pain}");
    assert!(generated_pain.contains("<MsgId>SEPA-TERM-MULTI</MsgId>"));
    assert!(generated_pain.contains("<PmtInfId>PMT-TERM-MULTI</PmtInfId>"));
    assert!(generated_pain.contains("<NbOfTxs>2</NbOfTxs>"));
    assert!(generated_pain.contains("<CtrlSum>32.30</CtrlSum>"));
    assert!(generated_pain.contains("<ReqdExctnDt>2026-04-20</ReqdExctnDt>"));
    assert!(!snapshot.keys().any(|key| key.starts_with("sepa.")));
    ...
    assert!(
        body.contains(
            "HKCME:3:1+DE02123456780000000000:MARKDEF1100+32,3:EUR++urn?:sepade?:xsd?:pain.001.001.02+@"
        ),
        "{body}"
    );
    assert!(body.contains("<MsgId>SEPA-TERM-MULTI</MsgId>"), "{body}");
    assert!(
        body.contains("<PmtInfId>PMT-TERM-MULTI</PmtInfId>"),
        "{body}"
    );
    assert!(body.contains("<NbOfTxs>2</NbOfTxs>"), "{body}");
    assert!(body.contains("<CtrlSum>32.30</CtrlSum>"), "{body}");
    assert!(body.contains("<ReqdExctnDt>2026-04-20</ReqdExctnDt>"));
    assert!(body.contains("<EndToEndId>E2E-TERM-1</EndToEndId>"));
    assert!(body.contains("<EndToEndId>E2E-TERM-2</EndToEndId>"));
    assert!(body.contains("<Ustrd>Term usage one</Ustrd>"), "{body}");
    assert!(body.contains("<Ustrd>Term usage two</Ustrd>"), "{body}");
}

#[tokio::test]
async fn handler_renders_and_collects_multi_last_sepa_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
        "HIRMG:2:2+0010::OK",
        "HIDME:3:1+ORDERMULTILAST",
    ]))]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler
        .new_job("MultiLastSEPA")
    ...
    job.try_set_indexed_param("btg.curr", 0, "EUR")
        .expect("first currency is accepted");
    job.try_set_indexed_param("usage", 0, "Debit usage one")
        .expect("first usage is accepted");
    job.try_set_indexed_param("endtoendid", 0, "E2E-LAST-1")
        .expect("first end to end id is accepted");
    job.try_set_indexed_param("creditorid", 0, "DE98ZZZ09999999999")
        .expect("first creditor id is accepted");
    job.try_set_indexed_param("mandateid", 0, "MND-1")
    ...
    job.try_set_indexed_param("btg.curr", 1, "EUR")
        .expect("second currency is accepted");
    job.try_set_indexed_param("usage", 1, "Debit usage two")
        .expect("second usage is accepted");
    job.try_set_indexed_param("endtoendid", 1, "E2E-LAST-2")
        .expect("second end to end id is accepted");
    job.try_set_indexed_param("creditorid", 1, "DE98ZZZ09999999999")
        .expect("second creditor id is accepted");
    job.try_set_indexed_param("mandateid", 1, "MND-2")
    ...
    job.try_set_param("sepaid", "SEPA-MULTI-LAST")
        .expect("sepa id is accepted");
    job.try_set_param("pmtinfid", "PMT-MULTI-LAST")
        .expect("payment info id is accepted");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "MultiLastSEPA");
    assert!(status.job_results[0].success);
    assert_eq!(
        status.job_results[0]
            .result_data
    ...
    assert_eq!(result.order_id.as_deref(), Some("ORDERMULTILAST"));

    let snapshot = handler
        .passport()
        .get_persistent_data("termlast_ORDERMULTILAST")
        .expect("multi last sepa persistent data");
    assert_eq!(
        snapshot.get("sepadescr").map(String::as_str),
        Some(PAIN_008_001_01_URN)
    );
    ...
    assert_eq!(snapshot.get("Total.curr").map(String::as_str), Some("EUR"));
    assert_eq!(
        snapshot.get("sepa.dst[1].name").map(String::as_str),
        Some("Debtor Two")
    );
    assert_eq!(
    ...
    let generated_pain = snapshot
        ...
        .expect("generated pain is persisted");
    assert!(generated_pain.starts_with("B<?xml"), "{generated_pain}");
    assert!(generated_pain.contains("<MsgId>SEPA-MULTI-LAST</MsgId>"));
    assert!(generated_pain.contains("<NbOfTxs>2</NbOfTxs>"));
    assert!(generated_pain.contains("<CtrlSum>32.30</CtrlSum>"));
    assert!(generated_pain.contains("<MndtId>MND-1</MndtId>"));
    assert!(generated_pain.contains("<MndtId>MND-2</MndtId>"));
    ...
    assert!(
        body.contains(
            "HKDME:3:1+DE02123456780000000000:MARKDEF1100+32,3:EUR++urn?:sepade?:xsd?:pain.008.001.01+@"
        ),
        "{body}"
    );
    assert!(body.contains("<MsgId>SEPA-MULTI-LAST</MsgId>"), "{body}");
    assert!(
        body.contains("<PmtInfId>PMT-MULTI-LAST</PmtInfId>"),
        "{body}"
    );
    assert!(body.contains("<NbOfTxs>2</NbOfTxs>"), "{body}");
    assert!(body.contains("<CtrlSum>32.30</CtrlSum>"), "{body}");
    assert!(
        body.contains("<EndToEndId>E2E-LAST-1</EndToEndId>"),
        "{body}"
    );
    assert!(
        body.contains("<EndToEndId>E2E-LAST-2</EndToEndId>"),
        "{body}"
    );
...
#[tokio::test]
async fn handler_renders_and_collects_multi_last_b2b_sepa_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
        "HIRMG:2:2+0010::OK",
        "HIBME:3:1+ORDERMULTIB2B",
    ]))]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler
        .new_job("MultiLastB2BSEPA")
    ...
    job.try_set_param("src.name", "Creditor Name")
        .expect("source name is accepted");
    job.try_set_indexed_param("dst.name", 0, "Debtor Company One")
        .expect("first debtor name is accepted");
    job.try_set_indexed_param("dst.iban", 0, "DE99123456780000000000")
        .expect("first debtor iban is accepted");
    job.try_set_indexed_param("dst.bic", 0, "DEUTDEDB277")
        .expect("first debtor bic is accepted");
    job.try_set_indexed_param("btg.value", 0, "42.10")
        .expect("first amount is accepted");
    job.try_set_indexed_param("btg.curr", 0, "EUR")
        .expect("first currency is accepted");
    job.try_set_indexed_param("usage", 0, "B2B debit usage one")
        .expect("first usage is accepted");
    job.try_set_indexed_param("endtoendid", 0, "E2E-B2B-1")
        .expect("first end to end id is accepted");
    job.try_set_indexed_param("creditorid", 0, "DE98ZZZ09999999999")
        .expect("first creditor id is accepted");
    job.try_set_indexed_param("mandateid", 0, "MND-B2B-1")
        .expect("first mandate id is accepted");
    job.try_set_indexed_param("manddateofsig", 0, "2026-01-02")
        .expect("first mandate date is accepted");
    job.try_set_indexed_param("dst.name", 1, "Debtor Company Two")
        .expect("second debtor name is accepted");
    job.try_set_indexed_param("dst.iban", 1, "DE77123456780000000000")
        .expect("second debtor iban is accepted");
    job.try_set_indexed_param("dst.bic", 1, "COBADEFFXXX")
        .expect("second debtor bic is accepted");
    job.try_set_indexed_param("btg.value", 1, "10.20")
        .expect("second amount is accepted");
    job.try_set_indexed_param("btg.curr", 1, "EUR")
        .expect("second currency is accepted");
    job.try_set_indexed_param("usage", 1, "B2B debit usage two")
        .expect("second usage is accepted");
    job.try_set_indexed_param("endtoendid", 1, "E2E-B2B-2")
        .expect("second end to end id is accepted");
    job.try_set_indexed_param("creditorid", 1, "DE98ZZZ09999999999")
        .expect("second creditor id is accepted");
    job.try_set_indexed_param("mandateid", 1, "MND-B2B-2")
        .expect("second mandate id is accepted");
    job.try_set_indexed_param("manddateofsig", 1, "2026-01-03")
        .expect("second mandate date is accepted");
    job.try_set_param("sepaid", "SEPA-MULTI-B2B")
        .expect("sepa id is accepted");
    job.try_set_param("pmtinfid", "PMT-MULTI-B2B")
        .expect("payment info id is accepted");
    job.try_set_param("targetdate", "2026-03-15")
        .expect("target date is accepted");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "MultiLastB2BSEPA");
    assert!(status.job_results[0].success);
    assert_eq!(
        status.job_results[0]
            .result_data
    ...
    assert_eq!(result.order_id.as_deref(), Some("ORDERMULTIB2B"));

    let snapshot = handler
        .passport()
        .get_persistent_data("termlast_ORDERMULTIB2B")
        .expect("multi last b2b sepa persistent data");
    ...
    let generated_pain = snapshot
        ...
        .expect("generated pain is persisted");
    assert!(generated_pain.starts_with("B<?xml"), "{generated_pain}");
    assert!(generated_pain.contains("<MsgId>SEPA-MULTI-B2B</MsgId>"));
    assert!(generated_pain.contains("<NbOfTxs>2</NbOfTxs>"));
    assert!(generated_pain.contains("<CtrlSum>52.30</CtrlSum>"));
    assert!(generated_pain.contains("<MndtId>MND-B2B-1</MndtId>"));
    assert!(generated_pain.contains("<MndtId>MND-B2B-2</MndtId>"));
    ...
    assert!(
        body.contains(
            "HKBME:3:1+DE02123456780000000000:MARKDEF1100+52,3:EUR++urn?:sepade?:xsd?:pain.008.001.01+@"
        ),
        "{body}"
    );
    assert!(body.contains("<MsgId>SEPA-MULTI-B2B</MsgId>"), "{body}");
    assert!(
        body.contains("<PmtInfId>PMT-MULTI-B2B</PmtInfId>"),
        "{body}"
    );
    assert!(body.contains("<NbOfTxs>2</NbOfTxs>"), "{body}");
    assert!(body.contains("<CtrlSum>52.30</CtrlSum>"), "{body}");
    assert!(
        body.contains("<EndToEndId>E2E-B2B-1</EndToEndId>"),
        "{body}"
    );
    assert!(
        body.contains("<EndToEndId>E2E-B2B-2</EndToEndId>"),
        "{body}"
    );
    assert!(
        body.contains("<Dbtr><Nm>Debtor Company One</Nm></Dbtr>"),
        "{body}"
    );
    assert!(
        body.contains("<Dbtr><Nm>Debtor Company Two</Nm></Dbtr>"),
        "{body}"
    );
    assert!(
        body.contains("<Ustrd>B2B debit usage one</Ustrd>"),
        "{body}"
    );
    assert!(
        body.contains("<Ustrd>B2B debit usage two</Ustrd>"),
        "{body}"
    );
}

#[tokio::test]
async fn handler_renders_last_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_ok_response())]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler.new_job("Last").expect("job is in registry");
    ...
    job.try_set_param("btg.curr", "EUR")
        .expect("amount currency is accepted");
    job.try_set_param("usage", "Direct debit usage one")
        .expect("first usage line is accepted");
    job.try_set_param("usage_2", "Direct debit usage two")
        .expect("second usage line is accepted");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "Last");
    assert!(status.job_results[0].success);
    assert!(status.job_results[0].result.is_none());
    assert!(
        !status.job_results[0]
...
#[tokio::test]
async fn handler_renders_and_collects_last_sepa_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
        "HIRMG:2:2+0010::OK",
        "HIDSE:3:1+ORDERLAST",
    ...
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler.new_job("LastSEPA").expect("job is in registry");
    job.try_set_param("src.iban", "DE02123456780000000000")
        .expect("source iban is accepted");
    job.try_set_param("src.bic", "MARKDEF1100")
        .expect("source bic is accepted");
    ...
    job.try_set_param("dst.bic", "DEUTDEDB277")
        .expect("debtor bic is accepted");
    job.try_set_param("dst.addr.country", "CH")
        .expect("debtor address country is accepted");
    job.try_set_param("dst.addr.line1", "Street 1")
        .expect("debtor address line1 is accepted");
    job.try_set_param("btg.value", "12.30")
        .expect("amount value is accepted");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "LastSEPA");
    assert!(status.job_results[0].success);
    assert_eq!(
        status.job_results[0]
            .result_data
    ...
    assert_eq!(result.order_id.as_deref(), Some("ORDERLAST"));

    let snapshot = handler
        .passport()
        .get_persistent_data("termlast_ORDERLAST")
        .expect("last sepa persistent data");
    assert_eq!(
        snapshot.get("sepadescr").map(String::as_str),
        Some(PAIN_008_001_01_URN)
    );
    let generated_pain = snapshot
        ...
        .expect("generated pain is persisted");
    assert!(generated_pain.starts_with("B<?xml"), "{generated_pain}");
    assert!(generated_pain.contains("<MsgId>SEPA-LAST</MsgId>"));
    assert!(generated_pain.contains("<PmtInfId>SEPA-LAST</PmtInfId>"));
    assert!(generated_pain.contains("<ReqdColltnDt>2026-03-15</ReqdColltnDt>"));
    assert!(generated_pain.contains("<MndtId>MND-123</MndtId>"));
    assert!(generated_pain.contains("<AmdmntInd>false</AmdmntInd>"));
    assert_eq!(
        snapshot.get("sepa.creditorid").map(String::as_str),
        Some("DE98ZZZ09999999999")
    );
    assert_eq!(
        snapshot.get("sepa.sequencetype").map(String::as_str),
        Some("FRST")
    );
    assert!(!snapshot.contains_key("orderid"));
    ...
    let body = String::from_utf8(requests[0].body.clone()).expect("request body is text");
    assert_signed_custom_msg_request(&body, "0", "1", 5);
    assert!(body.contains("HKDSE:3:1+DE02123456780000000000:MARKDEF1100+"));
    assert!(body.contains("urn?:sepade?:xsd?:pain.008.001.01"), "{body}");
    assert!(body.contains("<pain.008.001.01>"), "{body}");
    assert!(body.contains("<MsgId>SEPA-LAST</MsgId>"), "{body}");
    assert!(body.contains("<ReqdColltnDt>2026-03-15</ReqdColltnDt>"));
    assert!(body.contains("<Id>DE98ZZZ09999999999</Id>"));
    assert!(body.contains("<MndtId>MND-123</MndtId>"));
    assert!(body.contains("<Ustrd>Direct debit usage</Ustrd>"));
}

#[tokio::test]
async fn handler_renders_and_collects_last_cor1_sepa_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
        "HIRMG:2:2+0010::OK",
        "HIDSC:3:1+ORDERCOR1",
    ...
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler.new_job("LastCOR1SEPA").expect("job is in registry");
    job.try_set_param("src.iban", "DE02123456780000000000")
        .expect("source iban is accepted");
    job.try_set_param("src.bic", "MARKDEF1100")
        .expect("source bic is accepted");
    ...
    job.try_set_param("btg.value", "12.30")
        .expect("amount value is accepted");
    job.try_set_param("usage", "COR1 debit usage")
        .expect("usage is accepted");
    job.try_set_param("sepaid", "SEPA-COR1")
        .expect("sepa id is accepted");
    job.try_set_param("creditorid", "DE98ZZZ09999999999")
        .expect("creditor id is accepted");
    job.try_set_param("mandateid", "MND-COR1")
        .expect("mandate id is accepted");
    job.try_set_param("manddateofsig", "2026-01-02")
        .expect("mandate date is accepted");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "LastCOR1SEPA");
    assert!(status.job_results[0].success);
    assert_eq!(
        status.job_results[0]
            .result_data
    ...
    assert_eq!(result.order_id.as_deref(), Some("ORDERCOR1"));

    let snapshot = handler
        .passport()
        .get_persistent_data("termlast_ORDERCOR1")
        .expect("last cor1 persistent data");
    assert_eq!(
        snapshot.get("sepadescr").map(String::as_str),
        Some(PAIN_008_001_01_URN)
    );
    ...
    let generated_pain = snapshot
        ...
        .expect("generated pain is persisted");
    assert!(generated_pain.starts_with("B<?xml"), "{generated_pain}");
    assert!(generated_pain.contains("<MsgId>SEPA-COR1</MsgId>"));
    assert!(generated_pain.contains("<MndtId>MND-COR1</MndtId>"));
    ...
    let body = String::from_utf8(requests[0].body.clone()).expect("request body is text");
    assert_signed_custom_msg_request(&body, "0", "1", 5);
    assert!(body.contains("HKDSC:3:1+DE02123456780000000000:MARKDEF1100+"));
    assert!(body.contains("urn?:sepade?:xsd?:pain.008.001.01"), "{body}");
    assert!(body.contains("<pain.008.001.01>"), "{body}");
...
#[tokio::test]
async fn handler_renders_and_collects_inst_ueb_sepa_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
        "HIRMG:2:2+0010::OK",
        "HIIPZ:3:1+ORDERIPZ+4+2",
    ]))]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler.new_job("InstUebSEPA").expect("job is in registry");
    job.try_set_param("src.iban", "DE02123456780000000000")
        .expect("source iban is accepted");
    job.try_set_param("src.bic", "MARKDEF1100")
        .expect("source bic is accepted");
    ...
    job.try_set_param("btg.value", "12.30")
        .expect("amount value is accepted");
    job.try_set_param("usage", "Instant usage")
        .expect("usage is accepted");
    job.try_set_param("sepaid", "SEPA-INST")
        .expect("sepa id is accepted");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "InstUebSEPA");
    assert!(status.job_results[0].success);
    assert_eq!(
        status.job_results[0]
            .result_data
    ...
    assert_eq!(
        status.job_results[0]
            .result_data
            .get("content.orderstatus")
            .map(String::as_str),
        Some("2")
    );
    let Some(HbciJobResultData::InstUebSepa(result)) = status.job_results[0].result.as_ref() else {
        panic!("expected InstUebSepa result data");
    };
    assert_eq!(result.order_id.as_deref(), Some("ORDERIPZ"));
    assert_eq!(result.cancellation_code.as_deref(), Some("4"));
    assert_eq!(result.order_status.as_deref(), Some("2"));
    ...
    let body = String::from_utf8(requests[0].body.clone()).expect("request body is text");
    assert_signed_custom_msg_request(&body, "0", "1", 5);
    assert!(body.contains("HKIPZ:3:1+DE02123456780000000000:MARKDEF1100+"));
    assert!(body.contains("<MsgId>SEPA-INST</MsgId>"), "{body}");
    assert!(body.contains("<PmtInfId>SEPA-INST</PmtInfId>"), "{body}");
    assert!(body.contains("<EndToEndId>NOTPROVIDED</EndToEndId>"));
    assert!(body.contains("<Ustrd>Instant usage</Ustrd>"));
}

#[tokio::test]
async fn handler_renders_umb_sepa_with_generated_pain_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_ok_response())]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler.new_job("UmbSEPA").expect("job is in registry");
    job.try_set_param("src.iban", "DE02123456780000000000")
        .expect("source iban is accepted");
    job.try_set_param("src.bic", "MARKDEF1100")
        .expect("source bic is accepted");
    ...
    job.try_set_param("btg.value", "12.30")
        .expect("amount value is accepted");
    job.try_set_param("usage", "Account transfer usage")
        .expect("usage is accepted");
    job.try_set_param("sepaid", "SEPA-UMB")
        .expect("sepa id is accepted");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "UmbSEPA");
    assert!(status.job_results[0].success);
    assert!(status.job_results[0].result.is_none());
    assert_eq!(
        status.job_results[0]
    ...
    let body = String::from_utf8(requests[0].body.clone()).expect("request body is text");
    assert_signed_custom_msg_request(&body, "0", "1", 5);
    assert!(body.contains("HKCUM:3:1+DE02123456780000000000:MARKDEF1100+"));
    assert!(body.contains("<MsgId>SEPA-UMB</MsgId>"), "{body}");
    assert!(body.contains("<PmtInfId>SEPA-UMB</PmtInfId>"), "{body}");
    assert!(body.contains("<EndToEndId>NOTPROVIDED</EndToEndId>"));
    assert!(body.contains("<Ustrd>Account transfer usage</Ustrd>"));
}

#[tokio::test]
async fn handler_renders_and_collects_term_ueb_sepa_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
        "HIRMG:2:2+0010::OK",
        "HICSE:3:1+ORDERTERM",
    ...
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler.new_job("TermUebSEPA").expect("job is in registry");
    job.try_set_param("src.iban", "DE02123456780000000000")
        .expect("source iban is accepted");
    job.try_set_param("src.bic", "MARKDEF1100")
        .expect("source bic is accepted");
    ...
    job.try_set_param("btg.value", "12.30")
        .expect("amount value is accepted");
    job.try_set_param("usage", "Term usage")
        .expect("usage is accepted");
    job.try_set_param("sepaid", "SEPA-TERM")
        .expect("sepa id is accepted");
    job.try_set_param_date("date", "2025-12-15")
        .expect("date is accepted");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "TermUebSEPA");
    assert!(status.job_results[0].success);
    assert_eq!(
        status.job_results[0]
            .result_data
    ...
    assert_eq!(result.order_id.as_deref(), Some("ORDERTERM"));

    let snapshot = handler
        .passport()
        .get_persistent_data("termueb_ORDERTERM")
        .expect("term ueb persistent data");
    assert_eq!(
        snapshot.get("My.iban").map(String::as_str),
        Some("DE02123456780000000000")
    );
    ...
    let generated_pain = snapshot
        ...
        .expect("generated pain is persisted");
    assert!(generated_pain.starts_with("B<?xml"), "{generated_pain}");
    assert!(generated_pain.contains("<MsgId>SEPA-TERM</MsgId>"));
    assert!(generated_pain.contains("<ReqdExctnDt>2025-12-15</ReqdExctnDt>"));
    assert!(!snapshot.keys().any(|key| key.starts_with("sepa.")));
...
#[tokio::test]
async fn handler_renders_and_collects_term_ueb_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
        "HIRMG:2:2+0010::OK",
        "HITUE:3:4+ORDERTERM",
    ...
    job.try_set_param("btg.curr", "EUR")
        .expect("amount currency is accepted");
    job.try_set_param("usage", "Term usage one")
        .expect("first usage line is accepted");
    job.try_set_param("usage_2", "Term usage two")
        .expect("second usage line is accepted");
    job.try_set_param_date("date", "2026-03-15")
        .expect("date is accepted");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "TermUeb");
    assert!(status.job_results[0].success);
    assert_eq!(
        status.job_results[0]
            .result_data
    ...
    assert_eq!(snapshot.get("date").map(String::as_str), Some("2026-03-15"));
    assert_eq!(snapshot.get("key").map(String::as_str), Some("51"));
    assert!(!snapshot.contains_key("sepapain"));
    assert!(!snapshot.keys().any(|key| key.starts_with("sepa.")));
    ...
    assert!(
        body.contains(
            "HKTUE:3:4+1234567890::280:10020030+99887766::280:20030040+Receiver Name++42,:EUR+51++Term usage one:Term usage two+20260315'"
        ),
        "{body}"
    );
}
...
#[tokio::test]
async fn handler_renders_and_collects_term_ueb_edit_like_original() {
    let mut data = signed_pintan_data();
    data.persistent_data.insert(
        "termueb_ORDEROLD".to_owned(),
        BTreeMap::from([
            ("My.number".to_owned(), "1234567890".to_owned()),
            ("My.KIK.country".to_owned(), "DE".to_owned()),
            ("My.KIK.blz".to_owned(), "10020030".to_owned()),
    ...
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
        "HIRMG:2:2+0010::OK",
        "HITUA:3:4+ORDERNEW+ORDEROLD",
    ]))]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler.new_job("TermUebEdit").expect("job is in registry");
    job.try_set_param("orderid", "ORDEROLD")
        .expect("order id is accepted");
    job.try_set_param("usage", "Edited usage one")
        .expect("explicit usage overrides snapshot");
    job.try_set_param_date("date", "2026-04-20")
        .expect("explicit date overrides snapshot");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "TermUebEdit");
    assert!(status.job_results[0].success);
    assert_eq!(
        status.job_results[0]
            .result_data
    ...
    let Some(HbciJobResultData::TermUebEdit(result)) = status.job_results[0].result.as_ref() else {
        panic!("expected TermUebEdit result data");
    };
    assert_eq!(result.order_id.as_deref(), Some("ORDERNEW"));
    assert_eq!(result.order_id_old.as_deref(), Some("ORDEROLD"));
    ...
    let snapshot = handler
        .passport()
        .get_persistent_data("termueb_ORDERNEW")
        .expect("new term ueb persistent data");
    assert_eq!(
        snapshot.get("My.number").map(String::as_str),
        Some("1234567890")
    );
    ...
    assert_eq!(
        snapshot.get("usage.usage").map(String::as_str),
        Some("Edited usage one")
    );
    assert_eq!(
        snapshot.get("usage.usage_2").map(String::as_str),
        Some("Term usage two")
    );
    assert_eq!(snapshot.get("date").map(String::as_str), Some("2026-04-20"));
    assert_eq!(snapshot.get("key").map(String::as_str), Some("51"));
    assert!(!snapshot.contains_key("id"));
    assert!(!snapshot.keys().any(|key| key.ends_with(".id")));
    assert!(!snapshot.keys().any(|key| key.starts_with("sepa.")));
    assert!(
        handler
            .passport()
            .get_persistent_data("termueb_ORDEROLD")
            .is_some()
    );
    ...
    assert!(
        body.contains(
            "HKTUA:3:4+1234567890::280:10020030+99887766::280:20030040+Receiver Name++42,:EUR+51++Edited usage one:Term usage two+20260420+ORDEROLD'"
        ),
        "{body}"
    );
}
...
#[tokio::test]
async fn handler_rejects_term_ueb_edit_without_snapshot_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_ok_response())]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler.new_job("TermUebEdit").expect("job is in registry");
    ...
    let err = handler
        .try_add_to_queue(job)
        .expect_err("missing term ueb edit snapshot is rejected");

    assert_eq!(err.kind(), hbci4rust::HbciErrorKind::InvalidArgument);
    assert!(
        err.message()
            .contains("TermUebEdit requires persistent data for termueb_UNKNOWN"),
        "{err}"
    );
    assert_eq!(replay.requests().expect("requests").len(), 0);
}
...
#[tokio::test]
async fn handler_renders_and_collects_term_ueb_sepa_list_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let pain = concat!(
        "<?xml version=\"1.0\" encoding=\"UTF-8\"?>",
        "<Document xmlns=\"urn:sepade:xsd:pain.001.001.02\">",
        "<pain.001.001.02>",
        "<GrpHdr><InitgPty><Nm>Sender Name</Nm></InitgPty></GrpHdr>",
        "<PmtInf>",
        "<PmtInfId>PMT-TERMLIST</PmtInfId>",
        "<ReqdExctnDt>2026-03-15</ReqdExctnDt>",
        "<DbtrAcct><Id><IBAN>DE02123456780000000000</IBAN></Id></DbtrAcct>",
        "<DbtrAgt><FinInstnId><BIC>MARKDEF1100</BIC></FinInstnId></DbtrAgt>",
        "<CdtTrfTxInf>",
        "<PmtId><EndToEndId>E2E-TERMLIST</EndToEndId></PmtId>",
        "<Amt><InstdAmt Ccy=\"EUR\">42.00</InstdAmt></Amt>",
        "<CdtrAgt><FinInstnId><BIC>DEUTDEDB277</BIC></FinInstnId></CdtrAgt>",
        "<Cdtr><Nm>Receiver Name</Nm></Cdtr>",
        "<CdtrAcct><Id><IBAN>DE99123456780000000000</IBAN></Id></CdtrAcct>",
        "<RmtInf><Ustrd>Term list usage</Ustrd></RmtInf>",
        "</CdtTrfTxInf>",
        "</PmtInf>",
        "</pain.001.001.02>",
        "</Document>",
    ...
    let hicsb = format!(
        "HICSB:3:1+DE02123456780000000000:MARKDEF1100+urn?:sepade?:xsd?:pain.001.001.02+@{}@{}+ORDERLIST+N+J",
        pain.len(),
        pain
    );
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
    ...
    job.try_set_param("my.iban", "DE02123456780000000000")
        .expect("my iban is accepted");
    job.try_set_param("my.bic", "MARKDEF1100")
        .expect("my bic is accepted");
    job.try_set_param_date("startdate", "2026-01-01")
        .expect("startdate is accepted");
    job.try_set_param_date("enddate", "2026-12-31")
        .expect("enddate is accepted");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "TermUebSEPAList");
    assert!(status.job_results[0].success);
    assert_eq!(
        status.job_results[0]
            .result_data
    ...
    assert_eq!(entry.usage, ["Term list usage".to_owned()]);
    assert_eq!(entry.date.as_deref(), Some("2026-03-15"));
    assert_eq!(entry.orderid.as_deref(), Some("ORDERLIST"));
    assert!(entry.can_change);
    assert!(!entry.can_delete);
    ...
    assert_eq!(snapshot.get("sepapain").map(String::as_str), Some(pain));
    assert_eq!(snapshot.get("candel").map(String::as_str), Some("N"));
    assert_eq!(snapshot.get("canchange").map(String::as_str), Some("J"));
    assert!(!snapshot.contains_key("orderid"));
    assert!(!snapshot.keys().any(|key| key.starts_with("SegHead.")));
    ...
    assert!(
        body.contains(
            "HKCSB:3:1+DE02123456780000000000:MARKDEF1100+urn?:sepade?:xsd?:pain.001.001.02+20260101+20261231+10'"
        ),
        "{body}"
    );
}
...
#[tokio::test]
async fn handler_renders_and_collects_term_ueb_sepa_edit_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
        "HIRMG:2:2+0010::OK",
        "HICSA:3:1+ORDERNEW+ORDEROLD",
    ]))]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler
        .new_job("TermUebSEPAEdit")
    ...
    job.try_set_param("btg.value", "12.30")
        .expect("amount value is accepted");
    job.try_set_param("usage", "Term edit usage")
        .expect("usage is accepted");
    job.try_set_param("sepaid", "SEPA-TERM-EDIT")
        .expect("sepa id is accepted");
    job.try_set_param_date("date", "2026-01-15")
        .expect("date is accepted");
    job.try_set_param("orderid", "ORDEROLD")
        .expect("order id is accepted");
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "TermUebSEPAEdit");
    assert!(status.job_results[0].success);
    assert_eq!(
        status.job_results[0]
            .result_data
    ...
    let snapshot = handler
        .passport()
        .get_persistent_data("termueb_ORDERNEW")
        .expect("term ueb edit persistent data");
    assert_eq!(
    ...
    let generated_pain = snapshot
        ...
        .expect("generated pain is persisted");
    assert!(generated_pain.starts_with("B<?xml"), "{generated_pain}");
    assert!(generated_pain.contains("<MsgId>SEPA-TERM-EDIT</MsgId>"));
    assert!(generated_pain.contains("<ReqdExctnDt>2026-01-15</ReqdExctnDt>"));
    assert!(!snapshot.keys().any(|key| key.starts_with("sepa.")));
    ...
    let body = String::from_utf8(requests[0].body.clone()).expect("request body is text");
    assert_signed_custom_msg_request(&body, "0", "1", 5);
    assert!(body.contains("HKCSA:3:1+DE02123456780000000000:MARKDEF1100+"));
    assert!(body.contains("<MsgId>SEPA-TERM-EDIT</MsgId>"), "{body}");
    assert!(body.contains("<ReqdExctnDt>2026-01-15</ReqdExctnDt>"));
    assert!(body.contains("+ORDEROLD'"), "{body}");
}
...
#[tokio::test]
async fn handler_requests_phototan_callback_for_parseable_matrix_payload() {
    let _guard = RUNTIME_CALLBACK_TEST_LOCK.lock().await;
    done().expect("runtime reset");
    let events = Arc::new(Mutex::new(Vec::new()));
    init(
    ...
    let payload = ascii_image_payload();
    assert!(MatrixCode::try_parse(Some(payload.as_str())).is_some());
    let hitan = format!(
        "HITAN:3:5+1++ORDER-REF-PHOTO+Bitte photoTAN bestaetigen+@{}@{}",
        payload.len(),
        payload
    );
    let passport = passport_with_cached_pin(visual_tan_pintan_data("photoTAN", "photoTAN"));
    let replay = ReplayCommClient::new([
        Ok(custom_msg_response(&["HIRMG:2:2+0010::OK", hitan.as_str()])),
        Ok(custom_msg_response_for_request(
            "0",
            2,
            &["HIRMG:2:2+0010::OK"],
    ...
    let status = handler
        .execute_tan2step_process2_submission()
        .await
        .expect("photoTAN process-2 submission executes");

    assert!(status.success);
    let body = String::from_utf8(replay.requests().expect("requests")[1].body.clone())
        .expect("body is text");
    let sig_tail = fints_segment(&body, "HNSHA");
    assert_eq!(
        sig_tail.split('+').collect::<Vec<_>>().get(3).copied(),
        Some("12345:987654")
    );
    ...
    let event = events
        .iter()
        .find(|event| event.reason == CallbackReason::NeedPtPhotoTan)
        .expect("photoTAN callback event");
    assert_eq!(
        event.message,
        "photoTAN\nBitte bestaetigen\n\nBitte photoTAN bestaetigen"
    );
    assert_eq!(event.data_type, CallbackDataType::Text);
    assert_eq!(event.current_value.as_deref(), Some(payload.as_str()));
    assert!(
        events
            .iter()
            .all(|event| event.reason != CallbackReason::NeedPtTan)
...
#[tokio::test]
async fn handler_requests_qrtan_callback_for_parseable_qr_payload() {
    let _guard = RUNTIME_CALLBACK_TEST_LOCK.lock().await;
    done().expect("runtime reset");
    let events = Arc::new(Mutex::new(Vec::new()));
    init(
    ...
    let payload = ascii_image_payload();
    assert!(QrCode::try_parse(Some(payload.as_str()), Some("QR-TAN challenge")).is_some());
    let hitan = format!(
        "HITAN:3:5+1++ORDER-REF-QR+Bitte QR-TAN bestaetigen+@{}@{}",
        payload.len(),
        payload
    );
    let passport = passport_with_cached_pin(visual_tan_pintan_data("Q1S", "QR-TAN"));
    let replay = ReplayCommClient::new([
        Ok(custom_msg_response(&["HIRMG:2:2+0010::OK", hitan.as_str()])),
        Ok(custom_msg_response_for_request(
            "0",
    ...
    let status = handler
        .execute_tan2step_process2_submission()
        .await
        .expect("QR-TAN process-2 submission executes");
    ...
    let event = events
        .iter()
        .find(|event| event.reason == CallbackReason::NeedPtQrTan)
        .expect("QR-TAN callback event");
    assert_eq!(
        event.message,
        "QR-TAN\nBitte bestaetigen\n\nBitte QR-TAN bestaetigen"
    );
    assert_eq!(event.data_type, CallbackDataType::Text);
    assert_eq!(event.current_value.as_deref(), Some(payload.as_str()));
    assert!(
...
#[tokio::test]
async fn handler_falls_back_to_generic_tan_callback_for_invalid_qr_payload() {
    let _guard = RUNTIME_CALLBACK_TEST_LOCK.lock().await;
    done().expect("runtime reset");
    let events = Arc::new(Mutex::new(Vec::new()));
    init(
    ...
    let payload = "not-a-qr-payload";
    let hitan = format!(
        "HITAN:3:5+1++ORDER-REF-QR-FALLBACK+Bitte TAN eingeben+@{}@{}",
        payload.len(),
        payload
    );
    let passport = passport_with_cached_pin(visual_tan_pintan_data("Q1S", "QR-TAN"));
    ...
    let status = handler
        .execute_tan2step_process2_submission()
        .await
        .expect("fallback process-2 submission executes");
    ...
    let event = events
        ...
        .expect("generic TAN callback event");
    assert_eq!(
        event.message,
        "QR-TAN\nBitte bestaetigen\n\nBitte TAN eingeben"
    );
    assert_eq!(event.current_value.as_deref(), Some(payload));
    assert!(
        events
            .iter()
            .all(|event| event.reason != CallbackReason::NeedPtQrTan)
    );
    drop(events);
    done().expect("runtime reset");
}
...
#[tokio::test]
async fn handler_preserves_sca_state_after_failed_process2_tan_submission() {
    let _guard = RUNTIME_CALLBACK_TEST_LOCK.lock().await;
    done().expect("runtime reset");
    let events = Arc::new(Mutex::new(Vec::new()));
    init(
    ...
    let replay = ReplayCommClient::new([
        Ok(custom_msg_response(&[
            "HIRMG:2:2+0010::OK",
            "HITAN:3:5+1++ORDER-REF-FAILED+Bitte geben Sie die TAN ein+@5@HHDUC",
        ])),
        Ok(custom_msg_response_for_request(
            "0",
            2,
            &["HIRMG:2:2+9010::TAN ungueltig"],
        )),
    ]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut saldo = handler.new_job("SaldoReq").expect("job is in registry");
    ...
    let failed_status = handler
        .execute_tan2step_process2_submission()
        .await
        .expect("failed process-2 TAN submission returns status");

    assert!(!failed_status.success);
    assert_eq!(failed_status.job_results.len(), 1);
    assert_eq!(failed_status.job_results[0].job_name, "TAN2Step");
    assert!(!failed_status.job_results[0].success);
    assert_eq!(
        failed_status.messages,
        vec!["9010:TAN ungueltig".to_owned()]
    );
    assert_eq!(
        handler.passport().sca_state().order_ref.as_deref(),
        Some("ORDER-REF-FAILED")
    );
    assert_eq!(
    ...
    assert!(fints_segment(&second_body, "HKTAN").starts_with("HKTAN:3:5+2"));
    assert!(
        fints_segment(&second_body, "HKTAN").contains("ORDER-REF-FAILED"),
        "{second_body}"
    );
    assert_eq!(
        fints_segment(&second_body, "HNSHA")
    ...
    assert_eq!(tan_events[0].current_value.as_deref(), Some("HHDUC"));
    done().expect("runtime reset");
}
...
#[tokio::test]
async fn handler_renders_kontoauszug_pdf_request_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_ok_response())]);
    let mut handler = HbciHandler::with_comm("300", passport, replay.clone());
    let mut job = handler
    ...
    assert!(status.success);
    assert_eq!(status.job_results[0].job_name, "KontoauszugPdf");
    assert!(status.job_results[0].result.is_none());
    ...
    assert_signed_custom_msg_request(&body, "0", "1", 5);
    assert!(
        body.contains("HKEKP:3:2+DE02123456780000000000:MARKDEF1100:::280+3+2024+2+NEXT'"),
        "{body}"
    );
}

#[tokio::test]
async fn handler_collects_kontoauszug_pdf_result_like_original() {
    let passport = passport_with_cached_pin(signed_pintan_data());
    let replay = ReplayCommClient::new([Ok(custom_msg_response(&[
        "HIRMG:2:2+0010::OK",
        "HIEKP:3:2+@20@JVBERi0xLjQKYm9keQ==+20240101:20241231+20250102+2024+7+DE02123456780000000000+MARKDEF1100+Name One+Name Two+Name Three+statement.pdf+@7@RECEIPT",
    ]))]);
    let mut handler = HbciHandler::with_comm("300", passport, replay);
    let mut job = handler
        .new_job("KontoauszugPdf")
    ...
    assert!(status.success);
    let result = &status.job_results[0];
    assert_eq!(result.job_name, "KontoauszugPdf");
    assert_eq!(
        result.result_data.get("content.booked").map(String::as_str),
        Some("JVBERi0xLjQKYm9keQ==")
    );
    assert_eq!(
        result
            .result_data
    ...
    assert_eq!(entry.format, Some(KontoauszugFormat::Pdf));
    assert_eq!(entry.data.as_deref(), Some(&b"%PDF-1.4\nbody"[..]));
