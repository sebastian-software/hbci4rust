...
#[test]
fn multi_last_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);

    let err = handler
        .new_job("MultiLast")
        .expect_err("classic DTAUS bulk direct debit is outside v1 public registry");
    assert_eq!(err.message(), "unsupported or out-of-scope job: MultiLast");
}

#[test]
fn multi_ueb_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);

    let err = handler
        .new_job("MultiUeb")
        .expect_err("classic DTAUS bulk transfer is outside v1 public registry");
    assert_eq!(err.message(), "unsupported or out-of-scope job: MultiUeb");
}
