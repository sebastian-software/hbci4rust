...
#[test]
fn dauer_list_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let err = handler
        .new_job("DauerList")
        .expect_err("classic standing-order list is outside v1 public registry");
    assert_eq!(err.message(), "unsupported or out-of-scope job: DauerList");
}

#[test]
fn dauer_new_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let err = handler
        .new_job("DauerNew")
        .expect_err("classic standing-order creation is outside v1 public registry");
    assert_eq!(err.message(), "unsupported or out-of-scope job: DauerNew");
}

#[test]
fn dauer_edit_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let err = handler
        .new_job("DauerEdit")
        .expect_err("classic standing-order edit is outside v1 public registry");
    assert_eq!(err.message(), "unsupported or out-of-scope job: DauerEdit");
}

#[test]
fn dauer_del_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let err = handler
        .new_job("DauerDel")
        .expect_err("classic standing-order deletion is outside v1 public registry");
    assert_eq!(err.message(), "unsupported or out-of-scope job: DauerDel");
}
...
#[test]
fn term_ueb_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let err = handler
        .new_job("TermUeb")
        .expect_err("classic scheduled transfer is outside v1 public registry");
    assert_eq!(err.message(), "unsupported or out-of-scope job: TermUeb");
}

#[test]
fn term_ueb_del_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let err = handler
        .new_job("TermUebDel")
        .expect_err("classic scheduled-transfer deletion is outside v1 public registry");
    assert_eq!(err.message(), "unsupported or out-of-scope job: TermUebDel");
}

#[test]
fn term_ueb_edit_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let err = handler
        .new_job("TermUebEdit")
        .expect_err("classic scheduled-transfer edit is outside v1 public registry");
    assert_eq!(
        err.message(),
        "unsupported or out-of-scope job: TermUebEdit"
    );
}

#[test]
fn term_ueb_list_is_out_of_scope_in_public_registry() {
    let passport = PinTanPassport::new(PinTanPassportData::default());
    let handler = HbciHandler::new("300", passport);
    let err = handler
        .new_job("TermUebList")
        .expect_err("classic scheduled-transfer list is outside v1 public registry");
    assert_eq!(
        err.message(),
        "unsupported or out-of-scope job: TermUebList"
    );
}
