...
#[test]
fn extracts_values_from_gvres_syntax_function() {
    let syntax = load_protocol_spec("300")
        .expect("known protocol version loads")
        .parse_syntax()
        .expect("syntax parses");
    let message = parse_wire_message(
        "HNHBK:1:3+000000000123+300+DIALOG1+2+DIALOG0:1'HIRMG:2:2+0010::OK'HISAL:3:7+DE02123456780000000000:MARKDEF1100+Girokonto+EUR+C:123,45:EUR:20260605'HNHBS:4:1+2'",
    )
    .expect("wire message parses");
    let resolved = message
        .resolve_segments(&syntax)
    ...
    assert_eq!(
        values
            .get("CustomMsgRes.GVRes.SaldoRes7.SegHead.code")
            .map(String::as_str),
        Some("HISAL")
    );
    assert_eq!(
    ...
    assert_eq!(
        values
            .get("CustomMsgRes.GVRes.SaldoRes7.kontobez")
            .map(String::as_str),
        Some("Girokonto")
    );
    assert_eq!(
        values
            .get("CustomMsgRes.GVRes.SaldoRes7.booked.CreditDebit")
            .map(String::as_str),
        Some("C")
    );
    assert_eq!(
    ...
    assert_eq!(
        values
            .get("CustomMsgRes.GVRes.SaldoRes7.booked.date")
            .map(String::as_str),
        Some("2026-06-05")
    );
}
