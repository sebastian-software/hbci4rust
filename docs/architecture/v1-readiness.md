...
- static high-level job coverage is 63 of 68 upstream `GV*.java` classes, with
  the intentional `GVTemplate`, `GVLastCOR1SEPA`, `GVMultiLastCOR1SEPA`,
  `GVMultiLast`, and `GVMultiUeb` boundaries missing;
...
| High-level jobs | `src/gv/mod.rs`; `scripts/audit-job-coverage.sh`; `docs/architecture/job-coverage.md`; `docs/reference/unsupported-surfaces.md` | 63/68 upstream job classes covered; `GVTemplate`, `GVLastCOR1SEPA`, `GVMultiLastCOR1SEPA`, `GVMultiLast`, and `GVMultiUeb` are intentionally out of scope | Keep audit current after any registry or upstream-baseline change. |
