...
| License and attribution | ADR 0002; ADR 0249; `LICENSE`; `NOTICE`; `docs/reference/packaging.md` | LGPL-2.1-or-later assumption and upstream attribution points are recorded | Recheck before packaging/release if upstream headers create a distribution question. |
...
| Handler and dialog runtime | `src/manager/handler.rs`; dialog and PinTAN tests in `tests/bootstrap.rs`; connection lifecycle tests in `tests/runtime_callbacks.rs`; `docs/reference/error-reporting.md`; ADRs 0154-0167, 0238-0245, and 0250 | Dialog init/close, signed messages, TAN processes, SCA callbacks, QR/photoTAN callback emission, connection callbacks, failed process-2 submission retry state, process-2 transport retry state, decoupled status-request rendering, decoupled refresh polling, status/error reporting review, and replayed execution paths are substantially covered | Increase replay breadth and document new bank-specific error cases as they appear. |
...
| Java-to-Rust mapping | `docs/reference/java-to-rust-mapping.md`; `docs/reference/public-api.md`; `docs/reference/migration-examples.md`; `docs/reference/error-reporting.md`; `tests/public_api.rs`; ADRs 0241, 0247, 0248, and 0250 | Major concepts, v1 boundaries, crate-root API groups, handler flow, PinTAN execution guidance, callback reasons, status/error inspection, checked balance-request shape, and high-risk statement/SEPA migration examples are documented | Keep examples current as live-bank observations or migration questions expose unclear job shapes. |
...
| Release hardening | This page; `docs/architecture/release-checklist.md`; `docs/reference/packaging.md`; `docs/reference/passport-storage-security.md`; `docs/rustification/README.md`; porting plan; ADRs 0246, 0249, and 0251 | Remaining work is visible, package metadata/package-list review and storage security review are recorded, and v1 is not declared complete | Complete final release-candidate gates, upstream header recheck, and any explicit limitation docs. |
