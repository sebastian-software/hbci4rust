...
| Handler and dialog runtime | `src/manager/handler.rs`; dialog and PinTAN tests in `tests/bootstrap.rs`; ADRs 0154-0167 | Dialog init/close, signed messages, TAN processes, SCA callbacks, and replayed execution paths are substantially covered | Increase replay breadth and harden error/reporting behavior before calling v1 complete. |
