...
| Handler and dialog runtime | `src/manager/handler.rs`; dialog and PinTAN tests in `tests/bootstrap.rs`; connection lifecycle tests in `tests/runtime_callbacks.rs`; ADRs 0154-0167 and 0238-0242 | Dialog init/close, signed messages, TAN processes, SCA callbacks, connection callbacks, failed process-2 submission retry state, process-2 transport retry state, decoupled status-request rendering, and replayed execution paths are substantially covered | Increase replay breadth, finish decoupled refresh polling, and harden error/reporting behavior before calling v1 complete. |
